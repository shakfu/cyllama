...
- [ ] **Vendor-drift guard workflow (`check-vendor.yml` pattern)** -- llama.cpp's `check-vendor.yml` re-runs `scripts/sync_vendor.py` in CI and fails if the tree differs. cyllama's `git status` currently shows modified vendored headers (`mtmd.h`, `log.h`, `ggml-backend.h`, `common.h`) -- exactly the class of silent drift that caused the `GGML_MAX_NAME=128` ABI-match incident. Add `.github/workflows/check-thirdparty.yml` that runs the vendor sync step and diffs
...
- [ ] **Hourly warm-cache cron workflow (`build-cache.yml` pattern)** -- GitHub Actions caches evict after 7 days idle; cyllama's GPU wheel builds are manual `workflow_dispatch`, often weeks apart, so `deps-cuda-*` / `deps-rocm-*` are almost always cold. Add `.github/workflows/warm-cache.yml` on `cron: '0 * * * *'` that restores and re-saves each backend's dep cache
