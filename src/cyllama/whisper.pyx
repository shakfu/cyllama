...
class WhisperAheadsPreset:
    NONE = WHISPER_AHEADS_NONE
    N_TOP_MOST = WHISPER_AHEADS_N_TOP_MOST
    CUSTOM = WHISPER_AHEADS_CUSTOM
    TINY_EN = WHISPER_AHEADS_TINY_EN
    TINY = WHISPER_AHEADS_TINY
    BASE_EN = WHISPER_AHEADS_BASE_EN
    BASE = WHISPER_AHEADS_BASE
    SMALL_EN = WHISPER_AHEADS_SMALL_EN
    SMALL = WHISPER_AHEADS_SMALL
    MEDIUM_EN = WHISPER_AHEADS_MEDIUM_EN
    MEDIUM = WHISPER_AHEADS_MEDIUM
    LARGE_V1 = WHISPER_AHEADS_LARGE_V1
    LARGE_V2 = WHISPER_AHEADS_LARGE_V2
    LARGE_V3 = WHISPER_AHEADS_LARGE_V3
    LARGE_V3_TURBO = WHISPER_AHEADS_LARGE_V3_TURBO


class WhisperGretype:
    END = WHISPER_GRETYPE_END
    ALT = WHISPER_GRETYPE_ALT
    RULE_REF = WHISPER_GRETYPE_RULE_REF
    CHAR = WHISPER_GRETYPE_CHAR
    CHAR_NOT = WHISPER_GRETYPE_CHAR_NOT
    CHAR_RNG_UPPER = WHISPER_GRETYPE_CHAR_RNG_UPPER
    CHAR_ALT = WHISPER_GRETYPE_CHAR_ALT


# Python wrapper classes
cdef class WhisperContextParams:
    cdef whisper.whisper_context_params _c_params

    def __init__(self):
        self._c_params = whisper_context_default_params()
...
cdef class WhisperVadParams:
    cdef whisper_vad_params _c_params

    def __init__(self):
        self._c_params = whisper_vad_default_params()
...
cdef class WhisperFullParams:
    cdef whisper_full_params _c_params

    def __init__(self, strategy=WHISPER_SAMPLING_GREEDY):
        self._c_params = whisper_full_default_params(strategy)
    ...
    @strategy.setter
    def strategy(self, whisper_sampling_strategy value):
...
cdef class WhisperTokenData:
    cdef whisper_token_data _c_data
...
cdef class WhisperContext:
    ...
    def __init__(self, model_path, WhisperContextParams params=None):
        ...
        self._c_ctx = whisper_init_from_file_with_params(model_path_bytes, params._c_params)
    ...
    def __dealloc__(self):
        if self._c_ctx != NULL:
            whisper_free(self._c_ctx)
            self._c_ctx = NULL

    def version(self):
        return whisper_version().decode('utf-8')

    def system_info(self):
        return whisper_print_system_info().decode('utf-8')

    def n_vocab(self):
        return whisper_n_vocab(self._c_ctx)

    def n_text_ctx(self):
        return whisper_n_text_ctx(self._c_ctx)

    def n_audio_ctx(self):
        return whisper_n_audio_ctx(self._c_ctx)

    def is_multilingual(self):
        return bool(whisper_is_multilingual(self._c_ctx))

    def model_n_vocab(self):
        return whisper_model_n_vocab(self._c_ctx)

    def model_n_audio_ctx(self):
        return whisper_model_n_audio_ctx(self._c_ctx)

    def model_n_audio_state(self):
        return whisper_model_n_audio_state(self._c_ctx)

    def model_n_audio_head(self):
        return whisper_model_n_audio_head(self._c_ctx)

    def model_n_audio_layer(self):
        return whisper_model_n_audio_layer(self._c_ctx)

    def model_n_text_ctx(self):
        return whisper_model_n_text_ctx(self._c_ctx)

    def model_n_text_state(self):
        return whisper_model_n_text_state(self._c_ctx)

    def model_n_text_head(self):
        return whisper_model_n_text_head(self._c_ctx)

    def model_n_text_layer(self):
        return whisper_model_n_text_layer(self._c_ctx)

    def model_n_mels(self):
        return whisper_model_n_mels(self._c_ctx)

    def model_ftype(self):
        return whisper_model_ftype(self._c_ctx)

    def model_type(self):
        return whisper_model_type(self._c_ctx)

    def model_type_readable(self):
        return whisper_model_type_readable(self._c_ctx).decode('utf-8')

    def token_to_str(self, int token):
        cdef const char * result = whisper_token_to_str(self._c_ctx, token)
        if result == NULL:
            return ""
        return result.decode('utf-8')

    def token_eot(self):
        return whisper_token_eot(self._c_ctx)

    def token_sot(self):
        return whisper_token_sot(self._c_ctx)

    def token_solm(self):
        return whisper_token_solm(self._c_ctx)

    def token_prev(self):
        return whisper_token_prev(self._c_ctx)

    def token_nosp(self):
        return whisper_token_nosp(self._c_ctx)

    def token_not(self):
        return whisper_token_not(self._c_ctx)

    def token_beg(self):
        return whisper_token_beg(self._c_ctx)

    def token_lang(self, int lang_id):
        return whisper_token_lang(self._c_ctx, lang_id)

    def token_translate(self):
        return whisper_token_translate(self._c_ctx)

    def token_transcribe(self):
        return whisper_token_transcribe(self._c_ctx)

    def tokenize(self, str text, int max_tokens=512):
        cdef int n_tokens = 0
        cdef bytes text_bytes = text.encode('utf-8')
        cdef whisper_token * tokens = <whisper_token *>malloc(max_tokens * sizeof(whisper_token))

        try:
            n_tokens = whisper_tokenize(self._c_ctx, text_bytes, tokens, max_tokens)
            if n_tokens < 0:
    ...
    def token_count(self, str text):
        ...
        return whisper_token_count(self._c_ctx, text_bytes)
    ...
    def lang_id(self, str lang):
        ...
        return whisper_lang_id(lang_bytes)

    def lang_str(self, int id):
        cdef const char * result = whisper_lang_str(id)
        if result == NULL:
            return None
        return result.decode('utf-8')

    def lang_str_full(self, int id):
        cdef const char * result = whisper_lang_str_full(id)
        if result == NULL:
            return None
        return result.decode('utf-8')
    ...
    def encode(self, int offset=0, int n_threads=1):
        cdef int result = whisper_encode(self._c_ctx, offset, n_threads)
        if result != 0:
    ...
    def full_n_segments(self):
        return whisper_full_n_segments(self._c_ctx)

    def full_lang_id(self):
        return whisper_full_lang_id(self._c_ctx)

    def full_get_segment_t0(self, int i_segment):
        return whisper_full_get_segment_t0(self._c_ctx, i_segment)

    def full_get_segment_t1(self, int i_segment):
        return whisper_full_get_segment_t1(self._c_ctx, i_segment)

    def full_get_segment_text(self, int i_segment):
        cdef const char * result = whisper_full_get_segment_text(self._c_ctx, i_segment)
        if result == NULL:
            return ""
        return result.decode('utf-8')

    def full_n_tokens(self, int i_segment):
        return whisper_full_n_tokens(self._c_ctx, i_segment)

    def full_get_token_text(self, int i_segment, int i_token):
        cdef const char * result = whisper_full_get_token_text(self._c_ctx, i_segment, i_token)
        if result == NULL:
            return ""
        return result.decode('utf-8')

    def full_get_token_id(self, int i_segment, int i_token):
        return whisper_full_get_token_id(self._c_ctx, i_segment, i_token)

    def full_get_token_data(self, int i_segment, int i_token):
        cdef whisper_token_data c_data = whisper_full_get_token_data(self._c_ctx, i_segment, i_token)
    ...
    def full_get_token_p(self, int i_segment, int i_token):
        return whisper_full_get_token_p(self._c_ctx, i_segment, i_token)

    def full_get_segment_no_speech_prob(self, int i_segment):
        return whisper_full_get_segment_no_speech_prob(self._c_ctx, i_segment)

    def print_timings(self):
        whisper_print_timings(self._c_ctx)

    def reset_timings(self):
        whisper_reset_timings(self._c_ctx)

cdef class WhisperState:
    cdef whisper_state * _c_state
    cdef WhisperContext _ctx

    def __init__(self, WhisperContext ctx):
        self._ctx = ctx
        self._c_state = whisper_init_state(ctx._c_ctx)
    ...
    def __dealloc__(self):
        if self._c_state != NULL:
            whisper_free_state(self._c_state)
            self._c_state = NULL
