...
`GGML_NATIVE` tells ggml's cmake to optimize for the build machine's CPU (native ISA detection: AVX2, AVX-512, etc.) and, when CUDA is enabled, to set `CMAKE_CUDA_ARCHITECTURES=native` so only the installed GPU's SM architecture is targeted.
...
This has been `"75"` (sm_75 / Turing) since the first CUDA CI workflow. Any new defaults must not override these explicit settings.

---

## Observed sizes

Measured on an RTX 4060 (sm_89) with CUDA 12.0, dynamic build, `SD_USE_VENDORED_GGML=0`:
...
**1. Default `GGML_NATIVE=ON` for static builds (`get_backend_cmake_options`, ~line 1036):**
...
`build_shared()` already forces `GGML_NATIVE=OFF` after calling `get_backend_cmake_options()`, so the new default is harmless for dynamic builds.

**2. Default `CMAKE_CUDA_ARCHITECTURES=native` for dynamic builds (`build_shared`, ~line 1190):**
