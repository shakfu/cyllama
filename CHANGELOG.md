...
- **stable-diffusion.cpp upstream sync** -- Tracked two header additions in `thirdparty/stable-diffusion.cpp/include/stable-diffusion.h`. (1) New sampler `ER_SDE_SAMPLE_METHOD` added to `sample_method_t` (between `RES_2S` and `SAMPLE_METHOD_COUNT`) -- mirrored in `stable_diffusion.pxd` and exposed as `SampleMethod.ER_SDE` in `stable_diffusion.pyx`. The `cyllama.sd` CLI picks it up automatically via `SampleMethod[args.sampler.upper()]`, no enum list to maintain. (2) Two new capability-query APIs `sd_ctx_supports_image_generation` and `sd_ctx_supports_video_generation` -- declared in `.pxd` next to the other `sd_ctx_*` system-info functions and exposed as `SDContext.supports_image_generation` / `SDContext.supports_video_generation` properties (raises `RuntimeError` on uninitialized ctx, docstrings note the WAN-vs-SD/SDXL/FLUX split). Lets callers branch on model capability instead of attempting an image generation call against a video-only checkpoint and parsing the failure.
...
- **Strict mypy enabled across `src/`** -- `pyproject.toml` now sets `[tool.mypy] strict = true` and excludes `src/cyllama/_vendor/` (vendored jinja2/markupsafe). Touched ~35 source files annotating signatures, narrowing `Optional` lazy attributes (`LLM._ctx` / `_sampler` / `_cache` returned-from-ensure pattern), tightening `Dict`/`List` generics, and casting Cython return values to types. `agents/acp.py` `ContentBlock.text(...)` classmethod renamed to `from_text` (the original name shadowed the `text` dataclass field, which strict typecheck rejects -- callers in `acp.py` and `tests/test_acp.py` migrated). `llama/cli.py` `_generate_text` now returns the generated string instead of dropping it on the floor (was already computed, the missing return was a latent bug exposed by the annotation pass).

### Added

- **`AgentProtocol` -- structural contract for tool-calling agents** -- New `runtime_checkable` `Protocol` in `src/cyllama/agents/types.py` capturing the surface (`run(task) -> AgentResult`, `stream(task) -> Iterator[AgentEvent]`, `metrics: Optional[AgentMetrics]`) that the async wrappers (`AsyncReActAgent`, `AsyncConstrainedAgent`) and `ACPAgent._create_inner_agent` need from any inner agent. `ReActAgent`, `ConstrainedAgent`, and `ContractAgent` now explicitly inherit from it. The new `agents/types.py` module also holds the shared `AgentEvent` / `EventType` / `AgentMetrics` / `AgentResult` dataclasses (formerly in `agents/react.py`), so the cross-agent vocabulary lives independently of any one implementation -- `constrained.py` and `contract.py` no longer have to import from the ReActAgent module just to get the protocol contract. As a prerequisite, the duplicate `AgentMetrics` dataclass in `agents/constrained.py` was deleted in favour of re-exporting from `agents/types` (the two definitions were byte-identical). Cleans up the `cast(Optional[AgentMetrics], ...)` and `cast(Optional[List[Tool]], ...)` workarounds added during the strict-mypy pass: `AsyncConstrainedAgent.metrics` is now a plain return, and `ACPAgent._create_inner_agent` is typed `-> AgentProtocol` instead of `-> Any`. Exported as `cyllama.agents.AgentProtocol`.

- **`EmbedderProtocol` and `RAG(embedder=...)` injection point** -- New `runtime_checkable` `Protocol` in `src/cyllama/rag/types.py` (alongside the existing `SearchResult` / `Chunk` / `Document` / `EmbeddingResult` dataclasses) capturing the structural contract `RAGPipeline` and `RAG` actually require of an embedder: `dimension`, `embed`, `embed_batch`, `close`. Placed in the shared-vocabulary module so concrete embedder backends and consumers can import the contract without depending on any specific implementation; the default llama.cpp `Embedder` (`src/cyllama/rag/embedder.py`) now explicitly inherits from `EmbedderProtocol` so mypy enforces method-signature compatibility. Naming matches the parallel `src/cyllama/agents/types.py` home for `AgentProtocol` -- both packages use `types.py` as the shared-vocabulary file so the convention is consistent across the codebase. `RAG.__init__` gains an `embedder: EmbedderProtocol | None = None` parameter symmetric to the new `store=` parameter; passing one bypasses the default `Embedder(embedding_model)` construction (and ignores `embedding_model`). `RAGPipeline.__init__` retypes `embedder` to the protocol. Together with `VectorStoreProtocol`, this makes the whole RAG layer pluggable -- callers can mix and match the default sqlite store with OpenAI embeddings, or Qdrant with sentence-transformers, without forking. Exported as `cyllama.rag.EmbedderProtocol`.

- **`McpConnectionProtocol`** -- New `runtime_checkable` `Protocol` in `src/cyllama/agents/mcp.py` replacing the previous `McpConnection = Union[McpStdioConnection, McpHttpConnection]` alias. Both connection classes now explicitly inherit from the protocol; the `McpConnection` name is preserved as an alias for the protocol so existing internal references keep working. Lets the third planned transport (`McpTransportType.SSE` is already in the enum but unimplemented) drop in without touching `Union` literals. As a side benefit, `McpHttpConnection.send_request` was widened from `timeout: float = 30.0` to `timeout: Optional[float] = None` (None defaults to 30s internally) so its signature matches the protocol -- the underlying behaviour is unchanged for existing callers.

- **`VectorStoreProtocol`, `SqliteVectorStore`, and `RAG(store=...)` injection point** -- New `runtime_checkable` `Protocol` in `src/cyllama/rag/store.py` capturing the structural contract `RAGPipeline` and `RAG` actually require of a vector store: `search`, `add`, `is_source_indexed`, `get_source_by_label`, `clear`, `close`, `__len__`. The default sqlite-vector backend was renamed `VectorStore` -> `SqliteVectorStore` and now explicitly inherits from `VectorStoreProtocol` so mypy enforces method-signature compatibility (PEP 544 supports subclassing `runtime_checkable` protocols; the class behaves as a regular concrete type). `RAG.__init__` gains a `store: VectorStoreProtocol | None = None` parameter; passing one bypasses default sqlite construction (and ignores `db_path`), letting callers plug in alternative backends (Qdrant, Chroma, LanceDB, pgvector) without forking. `RAGPipeline.__init__` retypes `store` to the protocol. Backend-specific features (sqlite quantization, FTS5 hybrid search via `HybridStore`, raw `store.conn` SQL) remain on the concrete `SqliteVectorStore` and aren't part of the cross-backend interface. Backends without per-source dedup may return False/None from `is_source_indexed`/`get_source_by_label` -- the RAG layer treats that as "always re-index" and behaves correctly. Reference adapter (`cyllama/rag/stores/qdrant.py`) tracked as a TODO.

- **MCP client api to `LLM` (`api.py`)** -- The transports in `cyllama/agents/mcp.py` (stdio + HTTP, originally wired into the agent `Tool` abstraction) are now reachable from the top-level `LLM` API: `add_mcp_server(name, *, command/args/env/cwd | url/headers, transport=None, ...)` (lazy-creates an `McpClient`, infers transport, fail-fast connects), `remove_mcp_server`, `list_mcp_tools`, `list_mcp_resources`, `call_mcp_tool`, `read_mcp_resource`. New `chat_with_tools(messages, *, tools=None, use_mcp=True, max_iterations=8, ...)` drives a `ReActAgent` with merged caller-supplied + MCP tools (text-based ReAct loop works on any GGUF; resolves task from last user message, system prompt from leading system message). `LLM.close()` disconnects MCP best-effort. 15 tests in `tests/test_api_mcp.py` (no model required, fully mocked). MCP server direction (cyllama-as-server) deferred per the recommendation in `docs/dev/mcp.md` -- see that doc for the merits/counterweights analysis.

- **`docs/dev/mcp.md` -- MCP integration design** -- Two-direction proposal (cyllama as MCP client; cyllama as MCP server via stdio + Streamable-HTTP on `EmbeddedServer`) with a merits/counterweights analysis and recommended build order. Concludes that the client direction is the clearer win and the server direction should be deferred until either a concrete `transcribe`/`generate_image` consumer appears or the resource catalog unlocks a specific workflow.
...
- **CI `concurrency:` groups on all wheel workflows** -- `build-cibw.yml`, `build-gpu-wheels.yml`, and `build-new-wheels.yml` now set `concurrency: { group: ${{ github.workflow }}-${{ github.ref }}[-${{ inputs.link_mode }}], cancel-in-progress: true }`. A second dispatch (or push, once path-triggered) on the same branch cancels the in-flight matrix instead of letting it run to completion, recovering up to ~2h of CUDA/ROCm runtime per superseded run. GPU workflows also include `inputs.link_mode` in the group key so `static` and `dynamic` dispatches don't cancel each other. Pattern borrowed from upstream `llama.cpp/.github/workflows/release.yml:30-32`

- **`ggml-org/ccache-action` wired into wheel builds** -- `ccache-action@v1.2.16` (`evict-old-files: 1d`, keyed per job+link_mode) now runs before every Linux/macOS cibuildwheel invocation. For manylinux containers, ccache is installed via `CIBW_BEFORE_ALL_LINUX` (`yum/dnf install -y ccache` with `epel-release` where needed) and `CCACHE_DIR=/project/.ccache` is passed through `CIBW_ENVIRONMENT_LINUX`, which rides cibuildwheel's automatic workspace mount so the host-side cache-action save/restore sees the same directory the in-container compilers write to. `CMAKE_C_COMPILER_LAUNCHER=ccache` and `CMAKE_CXX_COMPILER_LAUNCHER=ccache` are propagated via each `CIBW_ENVIRONMENT_*`, plus `CMAKE_CUDA_COMPILER_LAUNCHER` (CUDA job) and `CMAKE_HIP_COMPILER_LAUNCHER` (ROCm job) so GPU compilers also hit the cache. Covers `build-cibw.yml` (Linux + macOS), `build-gpu-wheels.yml` (CUDA, ROCm, SYCL, Vulkan), and `build-new-wheels.yml` (macOS Intel Vulkan). Windows/MSVC ccache wiring deferred -- requires `/Z7` debug-format plumbing for ccache compatibility, tracked separately
...
- **`VectorStore` is deprecated; use `SqliteVectorStore` instead.** The legacy name is preserved as a deprecation alias so existing `from cyllama.rag import VectorStore` and `from cyllama.rag.store import VectorStore` imports keep working, but each import site now emits a `DeprecationWarning` once. Implemented via PEP 562 module-level `__getattr__` in `cyllama/rag/__init__.py` and `cyllama/rag/store.py` -- the warning fires only when the legacy name is actually requested, not on every `import cyllama.rag`, and `stacklevel=2` aims it at the user's import line rather than internal cyllama code. Removal date not yet committed; will track via the same TODO that adds the Qdrant adapter so the rename and the multi-backend story land together.
