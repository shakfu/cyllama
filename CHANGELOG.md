...
- **Backend-driven strict precheck in `wheel_repair`** -- `_run_wheel_repair` now fails fast with an actionable message when invoked for a GPU backend (`cuda`/`vulkan`/`hip`/`sycl`/`opencl`) but the corresponding `thirdparty/<llama,stable-diffusion,whisper>.cpp/dynamic/` build outputs are missing. Replaces the obscure auditwheel "cannot find dependency libllama.so.X" error with `wheel_repair --backend cuda requires dynamic build output, but these directories are missing: ... Build the dynamic deps first, e.g. GGML_CUDA=1 WITH_DYLIB=1 python scripts/manage.py build --all --deps-only --dynamic.` `cpu`/`metal` builds (where missing dynamic dirs are legitimate, e.g. CPU CIBW jobs) skip the precheck. Restores the friendly error the prior inline `test -d {project}/thirdparty/llama.cpp/dynamic || { echo ERROR; exit 1; }` CI snippet provided, but through the centralized surface.
...
- **llama.cpp updated to b9498 (from b9493)** -- no cyllama binding changes were required. The only public-header API change was a rename in `include/speculative.h`: `common_speculative_need_embd_pre_norm()` -> `common_speculative_need_embd_nextn()` (pre-norm -> nextn target embeddings); cyllama binds no `common_speculative_*` symbol, so nothing was affected. The b9493 `clip_n_mmproj_embd()` gemma4a source patch **remains necessary**: upstream has not merged it as of b9498 (a clean b9498 checkout still aborts when loading a gemma4a mmproj), so `_apply_source_patches()` continues to apply it.

- **stable-diffusion.cpp updated to master-669-2d40a8b (from master-652-92dc726)** -- the only public-API change was additive and non-breaking: a new `sd_vae_format_t` enum and two new `sd_ctx_params_t` fields, `vae_format` and `stream_layers`, both inserted mid-struct. No binding broke (Cython resolves struct offsets from the real header at compile time, and the wrapper builds the struct via `sd_ctx_params_init()`, so the new fields are correctly default-initialised). The two fields are now bound in `stable_diffusion.pxd` and exposed at the Python layer (see Added). No high-level API change.

- **llama.cpp updated to b9493 (from b9352)** -- no cyllama binding changes were required; the bound headers changed only additively (new multimodal projector types for gemma4 "ultra"/deepseek-ocr-2/exaone, a new `llama_context_params.n_outputs_max` field, and `llama_set_warmup` deprecated -- none of which cyllama sets or calls in a breaking way). One built-in chat template was added upstream (`granite-4.1`); `tests/test_chat.py::test_chat_builtin_templates` updated to match. **One upstream regression required a build-time source patch:** `clip_n_mmproj_embd()` in `tools/mtmd/clip.cpp` lost its `PROJECTOR_TYPE_GEMMA4A` case when the `GEMMA4UA` ("ultra" audio) variant was added, so loading a Gemma-4 audio mmproj (`clip.audio.projector_type = gemma4a`) and warming up the audio encoder falls through to `GGML_ABORT("Unknown projector type")` and aborts the process (SIGABRT) -- uncatchable from the Cython/Python layer. Fixed by the new `LlamaCppBuilder._apply_source_patches()` in `scripts/manage.py`, which idempotently applies every `scripts/patches/llama.cpp-*.patch` to the cloned source before building (self-disabling once upstream merges an equivalent fix). The patch groups `GEMMA4A` with `GEMMA4UA` (both return `hparams.projection_dim`, which equals the text model's embedding dim); patch file and rationale live in `scripts/patches/` and double as the upstream PR payload (refs ggml-org/llama.cpp#21325). **Still required as of b9498** -- upstream has not merged it.
