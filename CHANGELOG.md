...
- **GIL released on per-token hot paths (`tokenize`, `detokenize`, sampler `sample`/`accept`)** -- After the prior `llama_decode` / SD generation `nogil` change, the per-token loop in any streaming generator was still GIL-bound between decodes: every iteration calls `LlamaSampler.sample()` (which runs the full sampler chain — top_k, top_p, temp, dist, etc.), and prompt prefill begins with a `LlamaVocab.tokenize()` call that scans the entire input string. Both functions returned to Python under the GIL, blocking any other thread for the duration. Marked `nogil` in `src/cyllama/llama/llama.pxd` (`llama_tokenize`, `llama_detokenize`, `llama_sampler_sample`, `llama_sampler_accept`, `llama_sampler_apply`) and wrapped each call site in `src/cyllama/llama/llama_cpp.pyx` (`LlamaVocab.tokenize`, `LlamaVocab.detokenize`, `LlamaSampler.sample`, `LlamaSampler.accept`) in a `with nogil:` block, lifting all pointer/scalar locals out first. The buffer allocations and Python list-construction stay GIL-bound (where they belong); only the C work runs in the released region. Safe because the wrapped functions are pure C with no Python-callback hooks — they operate on token IDs, byte buffers, and float arrays. `llama_get_logits` / `llama_get_embeddings` were *not* changed: they're cheap pointer-return calls and the surrounding Python list-copy loop dominates anyway, so the nogil-acquire/release overhead would outweigh the benefit. Net effect on multi-threaded workloads: `LLM` instances driven from `asyncio.to_thread`/`ThreadPoolExecutor` no longer serialize per-token sampling against unrelated Python work; the cancel-via-`threading.Event` path in `LLM._generate_stream` polls more responsively because the sampler call no longer holds the GIL between the post-decode logits and the next iteration's `is_eog` check.

- **GIL released across long-running native calls (`llama_decode`, SD `generate_image`, SD `generate_video`)** -- The three biggest single-call native operations in the wrapper previously held the Python GIL for their full duration, blocking every other Python thread for the length of a decode (tens of ms to seconds for prompt prefill on a long context) or a diffusion run (seconds to minutes). `LlamaContext.decode` (`src/cyllama/llama/llama_cpp.pyx`) now mirrors the existing `encode` pattern: lift `ctx_ptr` and `c_batch` to local cdefs, call `llama_decode` inside a `with nogil:` block, then check the int result back in Python. `SDContext.generate_with_params` and `SDContext.generate_video_with_params` (`src/cyllama/sd/stable_diffusion.pyx`) do the same against `generate_image` / `generate_video`; both functions in `src/cyllama/sd/stable_diffusion.pxd` gained `nogil` on their declarations. SD's progress, log, and preview callback wrappers were already declared `noexcept with gil`, so they correctly re-acquire the GIL when invoked from inside the released region — no callback-side change required. `llama_decode` was already declared `nogil` in `llama.pxd`, so this is purely a call-site fix. User-visible effect: `asyncio.to_thread`, `ThreadPoolExecutor`, and any other concurrency that runs Python on one thread while generation runs on another now actually runs in parallel; the cancel-via-`threading.Event` path in `LLM._generate_stream` can now poll while a decode is in flight; SD's `_try_acquire_busy()` lock no longer effectively serializes everything across processes-pretending-to-be-threads.

### Documentation

- **`LLM.__init__` docstring spells out the kwargs/config merge semantics** -- The previous one-liner ("These override values in config if both are provided") left it ambiguous whether kwargs replaced the entire config or merged per-field, and whether the input ``config`` instance got mutated. Both are common misreadings (the original review of this codebase claimed full-replacement; that was wrong). Replaced with an explicit description: shallow per-field override, kwargs replace matching fields on a copy of ``config``, unnamed fields keep their original value, the input ``config`` is not mutated, and unknown kwarg names raise ``TypeError`` from the ``GenerationConfig`` dataclass constructor. Updated the corresponding example to demonstrate the partial-override case (set ``top_p`` on ``config``, override only ``temperature`` via kwargs, observe that ``top_p`` survives). Verified the documented behaviour by round-tripping a `GenerationConfig(temperature=0.9, top_p=0.95)` through ``LLM(model, config=config, temperature=0.5)``: result has ``temperature=0.5, top_p=0.95``, original config retains ``temperature=0.9``. No code change.

- **`WhisperContext` docstrings filled in** -- The class and ~30 of its methods previously had no docstrings, leaving Python users staring at a wall of identifiers like `full_n_segments`, `full_get_segment_t0`, `full_get_segment_text`, `token_eot`, `lang_str_full` with no hint of what they do. Names mirror the whisper.cpp C API one-to-one (intentional, so upstream docs map directly), but that convention is invisible from `dir(ctx)` or autocomplete. Added a class-level docstring explaining the C-API mirroring rule plus a worked end-to-end usage example, then per-method docstrings on the high-traffic methods: the `full_*` result-accessor family (segment/token text, ids, probabilities, timestamps -- with the **10 ms unit** caveat for `full_get_segment_t0` / `full_get_segment_t1` called out explicitly), `encode` (with a "use `full` instead" pointer), `tokenize` (with the buffer-too-small retry pattern documented), the `token_*` special-token getters (each labelled with the corresponding `<|...|>` whisper token), language helpers (`lang_id` / `lang_str` / `lang_str_full`), `is_multilingual`, model-introspection accessors, and `print_timings` / `reset_timings`. Verified via `WhisperContext.full_get_segment_t0.__doc__` etc. that the docstrings reach runtime. No code change.

- **SD struct-copy and preview-callback lifetime invariants documented in source** -- Two latent footguns in `src/cyllama/sd/stable_diffusion.pyx` had no in-source explanation, only the implicit knowledge that the surrounding code happens to keep references alive long enough. Added explicit comments describing each invariant so a future refactor doesn't silently introduce a use-after-free. (1) `SDContext.generate_with_params` does `params._params.sample_params = params._sample_params._params` -- a struct copy-by-value that aliases pointer fields (`guidance.slg.layers`, `custom_sigmas`) without deep-copying. The buffers are owned by the inner `_sample_params` Python object; the chain of references that keeps them alive (outer `params` -> `_sample_params` attribute -> heap buffers) is now spelled out, with a note on what would break the chain. (2) `_preview_callback_wrapper` builds `SDImage` views over `frames` with `owns_data=False`. The `frames` pointer is owned by stable-diffusion.cpp and only valid for the duration of the callback invocation; the next diffusion step reuses the buffer. Documented at both the cdef wrapper (for binding maintainers) and on the public `set_preview_callback` docstring (for end users) with concrete copy idioms (`img.save(path)`, `np.array(img.data, copy=True)`) for users who need to retain a frame past the callback return.

- **`CLAUDE.md` rewritten to match the actual codebase** -- The previous version described a nanobind / `_*_native.cpp` architecture (`_llama_native.cpp`, `_whisper_native.cpp`, `_sd_native.cpp` translation units exposed via `NB_MODULE`, with `*_cpp.py` Python facades re-exporting them) and pointed at a `NANOBIND.md` that does not exist in the repo. The actual project is Cython: `.pxd` cdef-extern blocks declare the C surface, `.pyx` files hold cdef classes and method implementations, `.pxi` files hold included sub-modules (mtmd, speculative, tts_helpers). Rewrote the Architecture Overview, Core Structure tree, Key Layers, and Adding-Bindings sections to reflect this. Added pointers to the now-established patterns in the codebase: `nogil` on long-running C declarations + `with nogil:` at call sites, `try/except BaseException` inside `noexcept` C->Python callback trampolines, and the `close()` + `__enter__`/`__exit__` idempotent-resource template. Documented the editable-install rebuild step that Cython edits require (`uv pip install --force-reinstall --no-deps -e .`). Updated the test count from "1150+" to the current 1367 passing. The stand-alone `docs/dev/nanobind.md` was left unchanged — it is correctly framed as future-migration analysis, not a description of present state.

### Added

- **`OpenAICompatibleClient` now supports `with` and `async with`** -- The OpenAI-API drop-in replacement at `src/cyllama/integrations/openai_compat.py:246` held an inner `LLM` via `self._generator` but did not expose any context-manager protocol. Users wanting deterministic GPU cleanup had to dig into the internal attribute (`client._generator.close()`) or rely on garbage collection. Added `close()`, `__enter__`/`__exit__`, and `__aenter__`/`__aexit__` that delegate to the underlying `LLM`. `close()` is idempotent (uses `getattr(self, "_generator", None)` so a partially-initialised client doesn't crash on cleanup). The async-exit path calls the synchronous `LLM.close()` directly rather than trampolining through the event loop, since closing an LLM is just freeing the C-side context and sampler -- no I/O. The existing `client.generator` accessor and the dominant non-context-manager usage are unchanged. Verified end-to-end: sync `with`, async `async with`, and double-close idempotency all behave as expected.

- **Whisper streaming callbacks now exposed on `WhisperFullParams`** -- whisper.cpp's three primary streaming hooks (progress percent, new-segment notifications, encoder-begin filter) were declared in `whisper.pxd` but never surfaced to Python. Streaming transcription was therefore impossible: callers had to let `whisper_full()` run silently to completion before reading any output. Added three setter methods (`set_progress_callback`, `set_new_segment_callback`, `set_encoder_begin_callback`) plus matching `cdef ... noexcept with gil` C trampolines (`_whisper_progress_trampoline`, `_whisper_new_segment_trampoline`, `_whisper_encoder_begin_trampoline`) in `src/cyllama/whisper/whisper_cpp.pyx`. Each trampoline reacquires the GIL across the C->Python boundary, retrieves the registered Python callable from the `WhisperFullParams` instance (passed as `user_data`), invokes it, and catches any user-callback exception with a traceback print (raising into the C decoder is undefined behaviour even under `noexcept with gil`). The progress callback receives an int in `[0, 100]`. The new-segment callback receives the number of segments added since the last invocation, so callers can `for i in range(ctx.full_n_segments() - n_new, ctx.full_n_segments()): print(ctx.full_get_segment_text(i))` for live transcript output. The encoder-begin callback returns a bool that gates the encoder pass -- useful for cooperative cancellation between chunks; on user-callback exception the trampoline returns True (proceed) so a buggy filter doesn't abort an in-flight transcription. All three accept None to clear a previously-registered callback. The `WhisperFullParams` instance is the user_data pointer, and the Python callables are stored as cdef object fields on the instance, so the lifetime invariant ("params must outlive the full() call") is satisfied automatically by the natural usage pattern (`ctx.full(samples, params)` holds a reference to params for the duration of the call). The fourth declared callback, `whisper_logits_filter_callback`, is intentionally not exposed in this change -- it requires zero-copy mutable buffer access in a per-token hot loop, and the design tradeoffs (numpy dependency, overhead model) deserve a separate decision. Verified end-to-end: a 3-second silence buffer fires progress at 0/100, new_segment once for 1 produced segment, encoder_begin once; clearing via `set_*_callback(None)` reliably removes the registration; raising in a callback prints the traceback and lets subsequent calls continue.

- **Context-manager protocol on `LlamaModel`, `LlamaContext`, `LlamaSampler`, `LlamaBatch`, `WhisperContext`** -- These five resource-owning cdef classes now implement `__enter__`/`__exit__` plus an idempotent `close()` method. Previously only `SDContext` and `Upscaler` supported `with` blocks; for the llama and whisper classes users had to rely on `del ctx; gc.collect()` (the discipline documented in `docs/dev/test-cleanup.md` and required by the 5-cycle Metal exhaustion rule in `tests/conftest.py`). With the new methods, deterministic cleanup at scope exit is just `with LlamaModel(...) as m: with LlamaContext(m, ...) as ctx: ...` and works through nested usage. `LlamaContext` and `LlamaBatch` previously had a `close()` that delegated to `__dealloc__()` directly -- both now do an explicit NULL-checked free + null-out (matching the existing `LlamaModel`/`LlamaSampler`/`WhisperContext` `__dealloc__` shape) so calling `close()` then later having `__dealloc__` run via GC no longer risks a second `*_free()` call. `LlamaBatch.__dealloc__` additionally now flips `self.owner = False` after freeing, closing a latent double-free path where the prior `close()` -> `__dealloc__()` -> later GC `__dealloc__()` would call `llama_batch_free` twice on the same struct (the other classes were already protected by their `ptr is NULL` check). All five `__exit__` methods return `False` so exceptions raised inside the `with` block propagate normally. Verified by nested-`with` smoke test (LlamaModel > LlamaContext > LlamaSampler > LlamaBatch all entered and exited cleanly) and a double-`close()` idempotency check. Full suite (1367 tests) still passes.

### Fixed

- **Bare `except Exception:` blocks in `api.py` no longer swallow real bugs silently** -- Five of seven catch-all blocks in `src/cyllama/api.py` previously did `except Exception: pass` with no diagnostic. Real failures (a transport disconnect bug, a wrapper-side `AttributeError` in the chat-template path, etc.) disappeared with no trace, leaving "it just used the wrong template, idk why" bug reports unanswerable. Tightened: (1) `__del__` cleanup at line 682 now wraps a `traceback.print_exc()` in a nested guard so the traceback surfaces during normal shutdown but is itself best-effort during interpreter teardown when even `print()` may fail. (2) `LLM.close()` MCP disconnect at 717 now uses `logger.warning(..., exc_info=True)`. (3) Tier 1 jinja2 chat-template fallback at 1348 (and the standalone variant at 1897) keep their broad catch -- the fallback chain depends on it -- but log at `debug` level with `exc_info=True`, so a wrapper bug that mimics a template error is now reachable by enabling debug logging on `cyllama.api` rather than invisible. The `_JinjaTemplateError` clause remains silent because that's the *expected* fallback trigger (Gemma's `raise_exception`, etc.), not an error. (4) `remove_mcp_server` conn-disconnect at 1595 now logs a warning while still completing the local registration removal. The remaining two broad catches were verified correct and got clarifying comments rather than narrowing: the streaming producer at 2336 must catch any exception to route it through the queue (narrowing would orphan exceptions and hang the consumer on `queue.get()`); the post-completion `producer_task.result()` at 2368 swallows because the exception was already routed via the queue path. No public API change; the only user-visible difference is that `cyllama.api` now emits debug/warning log records on previously-silent fallback paths.

- **`SDImageGenParams.set_hires_fix()` validates parameters up front instead of crashing the C library** -- The convenience method copied any combination of arguments straight onto the underlying `sd_hires_params_t` struct. Several misconfigurations made it through to `generate_image` and triggered undefined behavior in stable-diffusion.cpp: `upscaler=HiresUpscaler.MODEL` without a `model_path` (the C side dereferences a null path), a `model_path` that didn't exist on disk, negative `target_width`/`target_height`, `scale <= 0` when no absolute target was set, `denoising_strength` outside `[0.0, 1.0]`, an `upscaler` integer that didn't correspond to any `HiresUpscaler` enum value, or negative `steps`/`tile_size`. All are now caught in Python with a precise diagnostic naming the offending field and a hint at the fix (`"Either pass model_path=<path-to-upscaler> or pick a built-in upscaler (e.g. HiresUpscaler.LATENT)"`, `"Use 0 to derive from scale"`, etc.). Validation only runs when `enabled=True` so a disabled hires-fix block can carry placeholder values the user plans to flip on later. The `abs-target-bypasses-scale` case (`target_width > 0 and target_height > 0 and scale == 0`) is allowed since absolute dimensions override scale. No public API change; the dominant happy path (`set_hires_fix()` with defaults, or with a valid model path) is unchanged. All 131 SD tests still pass.

- **`WhisperContext.full()` now validates input arrays with clear error messages** -- The method binds its `samples` argument to a `float[::1]` Cython memoryview, which silently rejects bad inputs but with cryptic stock-Cython messages: a float64 numpy array surfaces as `"Buffer dtype mismatch, expected 'float' but got 'double'"`, a 2-D array as a shape-mismatch deep in the buffer protocol, a Python list as `"a bytes-like object is required, not 'list'"`. None of those tell the caller that the underlying requirement is "1-D C-contiguous float32 mono PCM at 16 kHz." `full()` now performs an explicit pre-flight check on numpy arrays (dtype, ndim, C-contiguity) and a generic empty-input check on any input type, with error messages that name the problem and the fix (e.g. `"samples must be a float32 array, got dtype=float64. Cast with samples.astype(np.float32) before calling."`). Non-numpy inputs continue to fall through to the Cython buffer-protocol path so `array.array`, `memoryview`, and other contiguous float32 buffers still work — only the diagnostic improves. numpy is imported lazily inside the function so callers who never reach `full()` don't pay for it. The previously-silent empty-array case (`len(samples) == 0` would dereference `&samples_view[0]` on a zero-length view) now raises `ValueError` up-front. No public API change; the dominant happy path (1-D float32 numpy array) is unchanged.

- **`BatchGenerator.generate_batch` no longer leaks pool slots on exception** -- `src/cyllama/batching.py:291-354` acquired a pooled `LlamaBatch` from `get_pooled_batch()` and only returned it via `return_batch_to_pool(batch)` on the success path at line 354. Any exception raised during the prompt-prefill `decode` (line 314), per-token `sample`/`decode` loop (lines 328 / 350), or `vocab.token_to_piece` decode would skip the return, stranding a batch slot for the lifetime of the process. Over a long-running server with `use_pooling=True` and intermittent decode failures (KV-slot OOM, cancellation, native crashes that get translated to Python exceptions), the pool would gradually drain to zero and every subsequent call would fall back to fresh allocation — the exact case the pool exists to avoid. Wrapped the entire generation body in `try: ... finally: if self.use_pooling: return_batch_to_pool(batch)` so the slot is always returned, including on exception. Pre-existing success-path semantics unchanged.

- **Async streaming `finally` blocks now cancel the producer task instead of awaiting it** -- Three `AsyncIterator` streaming methods (`AsyncReActAgent.stream`, `AsyncConstrainedAgent.stream` in `src/cyllama/agents/async_agent.py`; `AsyncRAG.stream` in `src/cyllama/rag/advanced.py`) followed the producer-task-plus-queue pattern but their `finally` block did a plain `await producer_task`. If the consumer broke out of the `async for` early (exception, generator close, timeout), `await producer_task` blocked the consuming coroutine until the underlying agent or retrieval generator finished naturally — which in turn held `self._lock`, so any other coroutine waiting on the same `AsyncReActAgent`/`AsyncRAG` instance was queued behind the abandoned stream. All three now follow the pattern already used in `api.py:2354-2369` (`AsyncLLM.stream`): if `producer_task.done()` is False, `cancel()` and `await` it under `except asyncio.CancelledError`; otherwise call `.result()` to surface (or discard) any pending exception. Note: `asyncio.to_thread` cannot actually cancel the underlying thread, so any in-flight LLM/retrieval work continues to run until it next yields control — but the consumer coroutine is no longer blocked, the lock is released promptly, and the next caller can proceed. The `AsyncLLM.stream` site was already correct (the original review missed it on first pass) and is unchanged.

- **`noexcept` C->Python callbacks no longer leave the error indicator set on user-callback failure** -- Four cdef wrappers in `src/cyllama/llama/llama_cpp.pyx` (`log_callback`, `abort_callback`, `sched_eval_callback`, `progress_callback`) are registered as C function pointers with ggml/llama.cpp and invoked from inside native code. They were declared `noexcept` (so a Python exception escaping them is officially undefined behavior) but their bodies called straight into `(<object>py_callback)(...)` with nothing catching a raise from the user's callback. Cython's `noexcept` would print and clear the exception, but only at the *next* interpreter checkpoint -- between the raise and that checkpoint, the Python error indicator is set, and any C code that does its own `PyErr_Occurred`-style check (or another callback firing first) sees stale state and behaves unpredictably. The `log_callback` was additionally fragile to non-UTF-8 log text: `text.decode()` defaulted to strict UTF-8 and would itself raise `UnicodeDecodeError` from inside a tight log path. All four wrappers now have an explicit `try: ... except BaseException: traceback.print_exc(); return <safe-default>` body. Defaults chosen per-callback so a buggy user callback fails-safe rather than fails-shut: `abort_callback` returns `False` (don't abort the in-flight decode), `sched_eval_callback` returns `False` (don't request tensor observation that won't be delivered), `progress_callback` returns `True` (don't abort model loading), `log_callback` is void and just swallows. `log_callback` also switched to `text.decode("utf-8", errors="replace")` so malformed log bytes don't themselves raise. Verified by registering a deliberately-raising log callback and loading a model: 607 callback invocations, every exception caught and traceback printed, process survived. Existing `_cancel_flag_callback` (pure-C `nogil`, no Python invocation) was already safe and is unchanged.

- **`LlamaContext` state serialization methods were silently broken** -- Three exported methods on `LlamaContext` (`src/cyllama/llama/llama_cpp.pyx`) violated the size-then-buffer pattern that `llama.h` requires, making them unusable: `get_state_data()` passed `dst=NULL, size=0` to `llama_state_get_data` so it copied zero bytes and the surrounding loop returned `[]` with no error -- callers thought they had a state snapshot but had nothing. `load_state_file()` and `load_state_seq_file()` passed `tokens_out=NULL` and `n_token_count_out=NULL` (a pointer-to-pointer typed as `size_t *` rather than the address of a real `size_t`) into C functions that dereference both, a NULL-deref segfault waiting for any non-empty session file; in the zero-token case the bool/size return was discarded and the empty list looked indistinguishable from success. All three now follow the same pattern already used correctly by `get_state_seq_data` in the same file: query the size (or accept a capacity), `malloc` a buffer of that size, pass it to the C call, copy out the actual number of bytes/tokens written, `free` the buffer in `finally`. The load methods now raise `RuntimeError` on C-side failure instead of silently returning `[]`, validate `max_n_tokens > 0`, and raise `MemoryError` on allocation failure. Verified end-to-end: a `get_state_data` -> `set_state_data` byte round-trip and a `save_state_file` -> `load_state_file` token round-trip both succeed against the default test model. No public API signature change. No existing tests covered these methods, which is why the bugs went undetected.
