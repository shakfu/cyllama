...
- **stable-diffusion.cpp updated to master-652-92dc726 (from master-612-d7ecbe1)** -- the upstream `generate_video` signature changed and required a binding update. It now returns `bool` (success) and writes its outputs through pointers -- `sd_image_t** frames_out`, `int* num_frames_out`, and a new `sd_audio_t** audio_out` -- rather than returning the frame array directly. `src/cyllama/sd/stable_diffusion.pxd` adds the `sd_audio_t` struct and `free_sd_audio()` declaration, imports `uint64_t`, mirrors the new `int fps` and `sd_hires_params_t hires` fields on `sd_vid_gen_params_t`, and updates the `generate_video` prototype. `src/cyllama/sd/stable_diffusion.pyx:generate_video` adapts to the out-param convention and frees any library-allocated audio (audio output is not yet exposed at the Python layer -- tracked in `TODO.md`), preserving the existing `List[SDImage]` return and per-frame cleanup. No high-level API change.
