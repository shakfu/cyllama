...
- **Experimental abi3 (stable-ABI) wheel build path** -- New `CYLLAMA_ABI3` CMake option (default `OFF`) wires every Cython extension through CPython's limited API so a single `cp310-abi3-<plat>` wheel is importable on Python 3.10+. Implementation is entirely additive: bumped `cmake_minimum_required` to 3.26 (needed by `USE_SABI`), conditionally requests `Development.SABIModule` in `find_package(Python ...)`, appends `-X limited_api=True` to the Cython transpile args when the option is on, and threads a `_SABI_ARGS` helper (`USE_SABI 3.10` or empty) through all four `python_add_library` calls (`add_cython_extension`, `embedded`, `whisper_cpp`, `stable_diffusion`). No `.pyx` files were modified — the `limited_api` directive is applied at build time via `CYTHON_ARGS`. Opt in locally with `pip install . --config-settings=cmake.define.CYLLAMA_ABI3=ON`; in CI via `SKBUILD_CMAKE_DEFINE=CYLLAMA_ABI3=ON` + `SKBUILD_WHEEL_PY_API=cp310`. Default per-version build path is unchanged.

- **Two experimental workflows for abi3 wheel builds** -- `.github/workflows/build-abi3.yml` (CPU, equivalent to `build-cibw.yml`) and `.github/workflows/build-gpu-wheels-abi3.yml` (CUDA/ROCm/SYCL/Vulkan Linux + CUDA/Vulkan Windows + Vulkan macOS-Intel, equivalent to `build-gpu-wheels2.yml`). Both set `CIBW_BUILD=cp310-*` and inject the two `SKBUILD_*` env vars into every `CIBW_ENVIRONMENT_*` block. Artifact names, cache keys, and release/index artifacts are namespaced with an `abi3` prefix so the flows coexist with the per-version workflows without collision. Smoke-test jobs add an `.abi3.` suffix guard that asserts every installed `.so`/`.pyd` carries the abi3 soabi tag, catching the silent case where `SKBUILD_WHEEL_PY_API=cp310` re-tags a regular cp310 wheel without the CMake option actually activating. The CPU workflow additionally verifies imports + one-token decode on a 3.10 x 3.11 x 3.12 x 3.13 x 3.14 matrix across Linux/macOS-arm/Windows — cross-version validation that cibuildwheel itself cannot perform on a single abi3 wheel. See `docs/dev/abi3.md` for the full plan, rollout steps, and known risks.
