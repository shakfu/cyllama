...
- **Vector store metadata-compatibility schema** - `VectorStore` now records the embedding model basename, embedding model file size, chunk size, chunk overlap, cyllama version, and created-at timestamp into the existing `{table_name}_meta` SQLite table alongside the dimension/metric/vector_type already stored. New constructor params `embedding_model_path`, `chunk_size`, `chunk_overlap` (all optional). On reopen of a populated DB, `_init_table` reads the stored metadata and verifies it matches the caller's config — mismatches raise `VectorStoreError` with a helpful message naming the stored value, the attempted value, and the fix (use the original config or `--rebuild`). Hard mismatches (dimension/metric/vector_type) always fire; soft mismatches (embedding model fingerprint, chunk config) only fire when the caller passes the corresponding field, so callers that don't care can opt out by simply not passing them. 11 new tests in `tests/test_rag_store.py::TestVectorStoreMetadataCompatibility`. `RAG.__init__` automatically forwards `embedding_model`, `chunk_size`, and `chunk_overlap` so the metadata is recorded on every persistent store created via the high-level API

- **Corpus deduplication via content hashing** - `VectorStore` gained a `{table_name}_sources` SQLite table that tracks `(content_hash, source_label, chunk_count, indexed_at)` for every source ever added. New `is_source_indexed`, `get_source_by_label`, and `list_sources` methods read it; new optional `source_hash` and `source_label` kwargs on `add()` write it atomically with the chunk inserts (the chunks-and-source pair is wrapped in a single SQLite transaction so a process death between writes can't leave the store with orphaned chunks). `RAG.add_documents` now hashes each file's bytes (md5) before indexing — files whose hash is already in the store are silently skipped. `RAG.add_texts` does the same for raw text strings. Both methods now return an `IndexResult` object that subclasses `list[int]` (preserving the existing `len()`/iter contract for callers doing `n_added += len(rag.add_documents(...))`) and additionally exposes `.skipped_labels` listing the basenames or text labels that were skipped. Files whose basename matches an existing source but whose content differs raise `ValueError` with a clear message pointing the user at `--rebuild` or rename — this catches the "I edited the file but kept the name" case where appending would silently produce two versions in the index. The `cyllama rag` CLI status line surfaces the skip count: `"reusing 128 chunks from rag.db (1 unchanged)"` after a re-run with the same `-f`, or `"3 new chunks appended to rag.db (5 unchanged)"` for a partial-skip append. Re-running `cyllama rag -f corpus.txt --db rag.db` after the first successful run is now a true no-op on the indexing side. 21 new tests in `tests/test_rag_dedup.py` and 9 new tests in `tests/test_rag_store.py::TestVectorStoreSourceDedup`
...
- **`cyllama rag` Qwen3 paragraph loops, `<think>` block leaks, and prompt-format meta-restarts** - The original raw-completion CLI default caused chat-tuned models to leak instruction-tuning artifacts (paragraph paraphrase loops, Qwen3 `<think>` block dumps, model re-roleplaying as user). Fixed at four layers: defaulting to chat-template path, the n-gram repetition detector as a safety net for residual lexical loops, the `ThinkBlockStripper` for reasoning blocks, and a strengthened default system prompt with `/no_think` directive for Qwen3. End-to-end verified against `Qwen3-4B-Q8_0.gguf` with `scripts/case/rag-chat1.sh`

- **`cyllama rag` interactive mode missing visual separation between turns** - Each answer was printed immediately followed by the next `> ` prompt. Now an unconditional `print()` at the end of each turn gives one blank line between turns

- **Leading blank line on every Qwen3 RAG answer** - `<think>` block stripping removed only the tagged content, not the `\n\n` Qwen3 wraps around it. Fixed by adding stateful leading-whitespace stripping in `ThinkBlockStripper` (re-armed at each close tag, handles whitespace spanning multiple chunks). 8 tests in `tests/test_rag_repetition.py::TestThinkBlockStripperLstrip`
