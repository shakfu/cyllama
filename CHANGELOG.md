...
- **vulkan-macos-intel smoke leg flagged delocate's bundled libraries as non-abi3** -- the abi3 suffix check in `_gpu-smoke.yml` and `build-cibw-abi3.yml` matched `*.dylib`, so it flagged the C libraries in `cyllama/.dylibs/`. Python extensions on macOS are `.so`.

- **SYCL smoke leg aborted, and the docs promised a CPU fallback that does not exist** -- ggml-sycl's device manager throws `can not find preferred GPU platform` when no GPU device exists, even with `ONEAPI_DEVICE_SELECTOR=opencl:cpu`, and nothing catches it. Any backend registration on a GPU-less host aborts: `import cyllama.sd`, `cyllama info`, `LLM()`. An Intel GPU is now documented as a hard requirement. Patching ggml-sycl to register zero devices was rejected: it adds a carried patch without making a GPU-less host useful. The SYCL leg now link-tests the `sd` extension instead of importing it and skips `cyllama info`. The OpenCL CPU runtime install and the 1-token CPU inference step are removed; `continue-on-error` had hidden that they could never pass.
...
- **Switching backends linked the next build against the previous backend's archives** -- `clean` deliberately spares `thirdparty/*/lib`, so a switch rebuilt the core archives in place and left the old backend's orphaned beside them. The mismatch surfaced as an undefined `ggml_*` symbol that killed `import cyllama` and every CLI entry point, and the dead archives were not free either: a CUDA-to-Vulkan switch left ~436 MB behind. All fourteen `build-<backend>` targets depend on `reset` now rather than `clean`.

- **Stale API documentation across `README.md` and `docs/`** -- fifteen documented names no longer resolved, one keyword argument was wrong, and six CLI example lines exited non-zero as written. The backend-introspection API had moved wholesale to the generated `build_config.json` and its accessor `cyllama._internal.build_config`; several `LlamaModel`, `LlamaContext` and `GGUFContext` members had been renamed upstream or never existed; the server and multimodal sections documented modules and functions that are not module-level names. Every replacement was verified against the built extension.

- **`cyllama.sd -v` shredded the sampling progress bar** -- the bar is drawn from C and withholds its newline until the final step, while verbose log lines come from Python's `sys.stdout`; the two writers shared a line with no protocol, so a 20-step run left a trail of half-drawn bars instead of one advancing in place. Both callbacks now erase before writing, and only when stdout is a tty.

### Changed

- **`scripts/rwt.py install --wheel` documented as the exception it is** -- `install --cuda` resolves to `cyllama-cuda12` on its own, but every example and hint led with the `--wheel cyllama-cuda12` spelling, which read as though the flag were required. The docs now lead with the bare backend flag. No behaviour changed.
