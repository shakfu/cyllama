...
- **`tests/test_memory_leaks.py`** - 3 new memory-leak regression tests covering loop create/destroy of `LLM`, `SDContext`, and `WhisperContext`. Each test warms up the allocator pool, samples ``ru_maxrss`` per cycle (no psutil dependency, matching `scripts/leak_check.py`), and asserts the peak RSS stays within a per-loader tolerance window (80 MB for LLM, 250 MB for SD's multi-GB model, 60 MB for whisper). All tests are marked `slow` and skip cleanly when the corresponding model file is missing; gated to non-Windows since `resource.getrusage` is POSIX-only (same gating as the existing `pytest-memray` dev dependency)

- **RAG repetition loop guard** - The Qwen3-4B paragraph-loop bug (TODO: "RAG response repetition") is fixed by two opt-in mechanisms in `cyllama.rag.pipeline.RAGPipeline`:
  - **Streaming-level n-gram detector** (`cyllama.rag.repetition.NGramRepetitionDetector`) — operates on word-normalised text in a rolling window. After each chunk, checks whether the trailing n-gram has appeared at least `threshold` times within the window; if so, the pipeline stops generation. Defaults are tuned (`window=300`, `ngram=5`, `threshold=3`) so paragraph-length loops are caught but normal English text does not false-positive. Configurable via `RAGConfig.repetition_window/ngram/threshold` (set `repetition_threshold > 0` to enable; default `0` = off for backward compatibility)
  - **Chat-template prompting path** — `RAGConfig.use_chat_template=True` makes the pipeline call `LLM.chat()` with a system + user message instead of sending a raw "Question:/Answer:" completion prompt. The default system prompt explicitly tells the model not to repeat or paraphrase its answer. Custom system prompt via `RAGConfig.system_prompt`. Lets chat-tuned models use their native instruction-following format and side-step the loop entirely
  - **CLI integration** — `cyllama rag` enables the detector by default (`--repetition-threshold 2 --repetition-ngram 5 --repetition-window 300`); library callers stay opt-in
  - **Fix is verified end-to-end against Qwen3-4B**: legacy path produces a 400-word bloated response that loops a ~50-word paragraph 4+ times until `max_tokens`; detector cuts this to ~115 words (−71%); chat-template path produces a single ~200-word coherent answer with no loop. Pinned by `tests/test_rag_integration.py::TestQwen3RAGLoopRegression`

- **`tests/test_rag_repetition.py`** - 14 new unit tests for `NGramRepetitionDetector`: validation, basic feed behaviour, punctuation/case normalisation, chunk-splitting words, rolling-window eviction, sticky `triggered` state, and realistic loop scenarios

- **`tests/test_rag_pipeline.py`** - 14 new mock-based tests for the new pipeline behaviour: `RAGConfig` validation of the new fields, legacy path preserved when detection is off, `query()` routing through streaming when detection is on, `stream()` early-stop on loop detection, `use_chat_template` routing through `generator.chat()` with system+user messages, and the combination of both features

- **`tests/test_rag_integration.py`** - 8 end-to-end integration tests using **real models and the real corpus** (`bge-small-en-v1.5-q8_0.gguf` + `Llama-3.2-1B-Instruct-Q8_0.gguf` + `tests/media/corpus.txt`). Covers the legacy path (preserves `GenerationStats`), chat-template path (`query` and `stream`), and detector false-positive checks against real generation. Includes a dedicated `TestQwen3RAGLoopRegression` class that uses `Qwen3-4B-Q8_0.gguf` to (a) reproduce the original paragraph-loop bug on the legacy path (~400 words), (b) prove the detector caps it at <200 words, and (c) prove the chat-template path produces a coherent answer mentioning Paris or Africa. Marked `slow`; skips cleanly when any required model is missing

- **`docs/dev/chat_templates.md`** - Maintainer guide to cyllama's chat-template system. Covers the user-facing API (`LLM.chat()`, `LLM._apply_template()`, the standalone `apply_chat_template()`), the four-path layered architecture (vendored jinja2 -> legacy substring heuristic -> pipeline system-merge -> pipeline raw-completion fallback) with a visual summary, the vendored `cyllama._vendor` setup including both import-path rewrites and why each is load-bearing, the re-vendoring procedure, extension points (custom globals, custom filters, `enable_thinking=False` for reasoning models), debugging recipes (inspecting rendered prompts, dumping embedded templates, identifying which fallback path fired, common failure modes including the `_MissingType` sentinel leak), a test-against-a-new-model recipe, and a file-and-line reference table. Preserves the original design analysis (six options A-F, libcommon.a stability constraint, trade-off rationale) as a "Historical context" appendix for maintainers who need to understand *why* the system is shaped this way before considering a change. Also documents four open follow-ups (`enable_thinking=False` for Qwen3, removing the now-mostly-dead legacy substring path, removing the pipeline-level path-4 raw-completion fallback, exposing `_apply_jinja_template` as a public method)

- **Vendored `jinja2` + `markupsafe` chat-template renderer** (Option F from `docs/dev/chat_templates.md`) - cyllama now ships pure-Python copies of `jinja2 3.1.6` and `markupsafe 3.0.3` (without the optional `_speedups.c` C extension) under `src/cyllama/_vendor/`, used for evaluating the model's embedded chat template via the new `LLM._apply_jinja_template` method. This handles **any** GGUF whose embedded chat template uses Jinja syntax that llama.cpp's basic C API doesn't recognise — Gemma 4 was the canonical bug (it uses `<|turn>` markers, not `<start_of_turn>`, which is what llama.cpp's `llm_chat_detect_template` substring heuristic looks for, so detection returned `LLM_CHAT_TEMPLATE_UNKNOWN` and `llama_chat_apply_template` returned -1 regardless of message shape). Other affected models include any future GGUFs whose embedded templates don't match one of llama.cpp's hardcoded substring heuristics. Vendoring rather than declaring `jinja2` as a runtime dependency keeps cyllama wheels self-contained (the entire vendor footprint is ~616 KB pure Python, negligible relative to cyllama's 30-90 MB compiled wheels) and gives byte-for-byte reproducibility across environments — every cyllama wheel of a given version uses exactly the same `jinja2`, regardless of what the user has installed in their own site-packages. Two import-path rewrites are applied to the vendored sources: (1) `from markupsafe import` → `from cyllama._vendor.markupsafe import` across 9 jinja2 files (8 of them rewritten by sed, plus 1 `import markupsafe` line), and (2) the literal `from jinja2.runtime import` emit string in `compiler.py:843` → `from cyllama._vendor.jinja2.runtime import` so compiled templates resolve runtime symbols from the vendored copy instead of leaking into a different jinja2 module via the top-level namespace. The second rewrite is load-bearing: without it, compiled templates pull runtime symbols from whatever `jinja2` is on `sys.path`, causing the cross-module identity check `if rv is missing:` between two different `missing` sentinels to fail and the `_MissingType` sentinel to leak into rendered output as the literal string `"missing"`. Both rewrites are scripted in `scripts/vendor_jinja2.sh` so re-vendoring is mechanical. Same `ImmutableSandboxedEnvironment` + `loopcontrols` extension setup that HuggingFace's `transformers.PreTrainedTokenizerBase.apply_chat_template` uses, with the same custom globals (`raise_exception`, `strftime_now`) and `tojson` filter. Verified end-to-end against `Llama-3.2-1B-Instruct-Q8_0.gguf` (canonical chat template), `Gemma-4-E4B-it-Q5_K_M.gguf` (the bug that motivated this), and the existing `test_chat.py` suite. Pinned by 15 new tests in `tests/test_jinja_chat.py` covering: vendored jinja2 import smoke test, undefined-variable rendering (catches the `_MissingType` regression), compiled-template runtime import path verification, the `set` inside `if not x is defined` pattern that real Llama-3 templates use, sandbox + loopcontrols extension wiring, `raise_exception` global propagation, end-to-end Llama-3 rendering with system + user messages and `add_generation_prompt`, multi-turn conversations, message validation errors, fallback wiring from `_apply_template` to the legacy substring-heuristic path on `template=` parameter, and a Gemma 4 regression class that loads the real model and runs end-to-end chat generation. The pipeline-level three-tier fallback (`canonical chat -> merged-user chat -> raw completion`) stays in place as the outermost safety net; with the vendored jinja2 path in place it now almost never fires, but is preserved for the rare case where `jinja2` itself can't evaluate a template (`TemplateSyntaxError`, malformed embedded template, etc.). License compliance: both `jinja2` and `markupsafe` are BSD-3-Clause, copies of their LICENSE files are at `src/cyllama/_vendor/LICENSE.jinja2` and `src/cyllama/_vendor/LICENSE.markupsafe`, vendoring policy and re-vendoring procedure documented in `src/cyllama/_vendor/README.md`

- **Readline-style line editing and persistent history in interactive REPLs** - The `cyllama rag` and `cyllama chat` interactive modes now support up/down arrow history cycling, in-line editing (left/right arrows, Ctrl-A / Ctrl-E, Ctrl-R reverse-search), and persistent history files across sessions. Implementation is the standard library's `readline` module: just importing it transparently upgrades Python's built-in `input()` to use libreadline (or libedit on macOS) for line editing with no other code changes required. New helper module `cyllama._readline` provides `setup_history(path, max_entries=1000)` (loads any existing history from `path`, registers an `atexit` handler to write back the truncated history on shutdown, gracefully no-ops on platforms where `readline` isn't available — notably Windows without `pyreadline3`) and `history_path_for(command)` (returns the canonical per-subcommand history file path, currently `~/.cyllama_rag_history` and `~/.cyllama_chat_history`, separate per command so the two REPLs don't pollute each other). History files are created with mode 0600 by readline so only the user can read them. Pinned by 13 new tests in `tests/test_readline.py` covering: path-naming convention (rag vs chat distinct), parent-directory creation, loading existing history files, missing-file as first-run case, corrupted-file graceful handling, max-entries cap on in-memory history, tilde expansion of HOME-relative paths, full session round-trip (write -> clear -> reload), and max-entries truncation on save
...
- **`cyllama rag` CLI default `--repetition-threshold` lowered from 3 to 2** - With the previous default, the detector required three full occurrences of a 5-gram inside the rolling window before stopping generation, which for paragraph-length loops needed roughly 330+ words to accumulate. The CLI's default `--max-tokens 200` cuts the model off at ~150-185 words, so the third repeat never started and the detector never fired, leaving the user staring at the very loop the guard was supposed to catch. Lowering the threshold to 2 means the detector fires on the *first* repeat of any 5-word phrase, which empirically catches the Qwen3-4B "Summarize the story" loop after just a handful of tokens into the second paragraph. Pinned by `tests/test_rag_integration.py::TestQwen3RAGLoopRegression::test_cli_default_combo_stops_loop`, which uses the exact CLI defaults so a future regression in the defaults is caught here. Verified end-to-end against `Qwen3-4B-Q8_0.gguf` with `scripts/case/rag-chat1.sh`: the "Summarize the story" prompt now returns a coherent two-paragraph summary (~230 words of substance: a plot paragraph followed by a themes paragraph) and the detector cuts the model off at the exact moment paragraph 3 starts repeating ("The"), instead of returning a half-paragraph repetition of paragraph 1. Note: with the loop suppressed, `max_tokens=200` becomes the binding constraint rather than the loop -- the saved budget is now spent on substance instead of paraphrase, but a fully-finished second paragraph requires bumping `--max-tokens`

- **`cyllama rag` CLI now defaults to the chat-template path** (`use_chat_template=True`); legacy raw-completion path is opt-in via the new `--no-chat-template` flag. Nearly every GGUF used with `cyllama rag` in practice is a chat-tuned model (Llama-3-Instruct, Qwen3-Chat, Mistral-Instruct, etc.), and the previous raw-completion `Question:/Context:/Answer:` template was causing those models to leak instruction-tuning artifacts into the output: Qwen3-4B dumping its `<think>` reasoning block (`"Okay, let me try to figure out how to summarize this story..."`), the model re-roleplaying as the user and re-asking the question, and various paraphrase-loop pathologies that the streaming repetition detector was patching after the fact. Routing through the model's native chat template (system + user messages via `LLM.chat()`) lets each model stay inside the prompt format it was instruction-tuned on, which sidesteps these failure modes at the source. The streaming repetition detector remains on as a belt-and-suspenders safety net for any residual lexical loops. The `--system` flag now routes to `RAGConfig.system_prompt` (the native system message) when the chat-template path is active, and continues to route to the prompt template only when `--no-chat-template` is passed. Library callers (`RAGConfig` directly) still default to `use_chat_template=False` for backward compatibility -- only the CLI flips. Pinned by 4 new tests in `tests/test_main.py::TestCmdRag`: `test_chat_template_on_by_default`, `test_no_chat_template_opts_out`, `test_system_routed_to_system_prompt_in_chat_mode`, `test_system_routed_to_prompt_template_in_legacy_mode`

- **`<think>...</think>` reasoning-block stripping for RAG** - New `cyllama.rag.repetition.ThinkBlockStripper` class implements a stream-safe filter that removes chain-of-thought reasoning blocks emitted by Qwen3, DeepSeek-R1, and other reasoning-tuned models invoked via their native chat template. The stripper is a state machine, not a regex pass: it correctly handles the `<think>` and `</think>` tags being split across streaming chunk boundaries (e.g. `"<thi"` then `"nk>"`), multiple blocks per stream, unclosed blocks (everything after the open tag is discarded -- judged correct because the model is broken), one-character-per-chunk worst case, and leading-whitespace stripping at the start of the stream and immediately after every `</think>` close tag (Qwen3 wraps its `<think>` block in `\n` characters, so the close tag is followed by `\n\n` that would otherwise survive into the user-facing output as a leading blank line on every answer). Wired into `RAGPipeline._generate_chunks` as the outermost filter -- the strip runs *before* the n-gram repetition detector so the detector's rolling window is not polluted with reasoning content the user never sees, and cannot false-positive on phrases the model legitimately repeats between its `<think>` block and its answer. Filter order is pinned by `tests/test_rag_pipeline.py::TestRAGPipelineThinkStripping::test_strip_then_detect_filter_order`. Configurable via `RAGConfig.strip_think_blocks` (default `False` at the library level for backward compatibility -- enabling forces the streaming code path which loses `GenerationStats`). The CLI flips this to `True` by default and exposes `--show-think` as an opt-out for users who want the reasoning visible (debugging or transcript capture). The default `RAG_SYSTEM_PROMPT` was also strengthened with an explicit "answer directly without showing any chain-of-thought" instruction plus the Qwen3-specific `/no_think` literal directive (other models ignore it as harmless text), so the model is asked to skip reasoning at the source AND any reasoning that slips through is stripped on its way to the user. 14 new tests in `tests/test_rag_repetition.py::TestThinkBlockStripper*` cover the stripper internals; 5 new tests in `tests/test_rag_pipeline.py::TestRAGPipelineThinkStripping` cover the pipeline wiring and the legacy-path strip-alone routing case. 2 new tests in `tests/test_main.py::TestCmdRag` (`test_strip_think_blocks_on_by_default`, `test_show_think_opts_out_of_strip`) pin the CLI default policy

### Fixed

- **`cyllama rag` CLI loop guard ineffective at default `--max-tokens 200`** - The streaming-level n-gram repetition detector enabled by default in the CLI never fired against the Qwen3-4B paragraph-loop bug because its threshold (3) required more tokens than the default `--max-tokens` budget (200) could produce. The model would emit a complete summary paragraph followed by a half-rendered repeat of the same paragraph, and the user would see the loop the guard was supposed to suppress. See the `Changed` entry above for the fix and the end-to-end verification

- **`cyllama rag` CLI leaking Qwen3 `<think>` reasoning blocks and meta-restarts** - With the legacy raw-completion path, chat-tuned models would emit a coherent answer followed by their instruction-tuning artifacts: Qwen3-4B in particular would print its full first paragraph and then start a *second* turn ("Okay, let me try to figure out how to summarize this story based on the context provided. The user wants a summary, so I need to extract the main points from..."), as if continuing a multi-turn conversation. The streaming repetition detector did not fire because there was no lexical loop -- it was a prompt-format mismatch causing the model to meta-restart inside its own answer. Fixed by defaulting the CLI to the chat-template path; see the `Changed` entry above

- **Qwen3 `<think>` reasoning blocks consuming the entire `--max-tokens` budget** - After the CLI started defaulting to the chat-template path (above), Qwen3-4B began emitting its full chain-of-thought block before any user-facing answer, and the block routinely consumed the entire 200-token budget on its own -- leaving zero tokens for the actual answer. Q1 ("Summarize the story") and Q3 ("Does he die in the story?") in the verification run produced *only* the `<think>` block with no answer at all, and Q2 ("is she a nurse?") produced the block plus a one-sentence answer. Fixed at three layers: (a) the new `ThinkBlockStripper` removes the blocks from the streamed output regardless of model, (b) the strengthened default system prompt asks the model not to emit reasoning at all (with the Qwen3-specific `/no_think` literal directive included for models that honour it), and (c) the CLI defaults to strip-on with `--show-think` opt-out. See the `Changed` entry above for the implementation details

- **Leading blank line on every Qwen3 RAG answer** - Even with `<think>...</think>` blocks correctly stripped, every Qwen3 answer in the CLI started with a `\n\n` lead-in: Qwen3 wraps its `<think>` block in newlines, so the `</think>` close tag is immediately followed by `\n\n` before the actual answer text. The previous strip removed only the tagged content, not the surrounding whitespace. Fixed by adding leading-whitespace stripping at the start of the stream and on every inside->outside transition inside `ThinkBlockStripper`. The strip is stateful (a `_pending_lstrip` flag re-armed at each close tag) so it correctly handles whitespace that spans multiple streaming chunks (e.g. `"\n"` then `"\n"` then `"The"`) and only consumes leading whitespace, not whitespace mid-content. 8 new tests in `tests/test_rag_repetition.py::TestThinkBlockStripperLstrip` cover the lstrip semantics including chunk-split whitespace, re-arming after multiple think blocks, mid-content whitespace preservation, and pure-whitespace streams

- **`cyllama rag` interactive mode missing visual separation between turns** - The interactive REPL printed each answer immediately followed by the next prompt (`> `) on the next line, with no blank line in between, making multi-turn conversations cramped to read. The previous code's only blank line came from a `print()` *inside* the `--sources` block, so it appeared only when sources were enabled. Now an unconditional `print()` at the end of each turn gives one blank line between every answer and the next prompt

- **`cyllama rag` crash on Gemma and other models that reject the `system` chat role** - Several chat templates distributed as GGUF (Gemma 2/3, some older Mistral instruct variants, a handful of community fine-tunes) only support `user` and `assistant` roles -- they reject a `system` message outright with `RuntimeError: Failed to apply chat template`. The new chat-template default in `cyllama rag` was always emitting `[system, user]`, so the first query against any such model crashed before generating a single token. Fixed in `RAGPipeline._chat_with_fallback`: try the canonical `[system, user]` shape first, catch `RuntimeError` whose message mentions "template", and retry with the system content merged into the first user message. The probe result is cached on `self._system_role_supported` (tri-state: `None` unknown, `True` canonical works, `False` use merged) so subsequent queries against the same pipeline instance skip straight to the working shape -- the try/except is paid exactly once per RAGPipeline lifetime. Unrelated `RuntimeError`s (e.g. `CUDA out of memory`, model crashes) are re-raised unchanged so genuine bugs aren't masked. The merge helper `_merge_system_into_user` is also a static utility that other callers can use directly. 6 new tests in `tests/test_rag_pipeline.py::TestRAGPipelineSystemRoleFallback` cover: canonical-path success, fallback on template error, decision caching across queries, unrelated-error propagation, and the merge helper's immutability and no-system-message edge cases

- **`cyllama rag` crash on models whose embedded Jinja chat template can't be applied at all** - Some GGUFs (Gemma 4 in particular, plus any model whose embedded Jinja template doesn't match one of the substring heuristics in llama.cpp's `llm_chat_detect_template`) cause `llama_chat_apply_template` to return -1 regardless of message shape. This is because cyllama's Cython binding calls llama.cpp's basic C API `llama_chat_apply_template`, which only handles a hardcoded set of templates detected via substring heuristics -- it does NOT actually evaluate Jinja. There's a separate `common_chat_apply_template` in `common/chat.cpp` with full Jinja support via `minja`, but cyllama doesn't link against that (`libcommon.a` is not a stable API and would break on every llama.cpp upgrade). The earlier system-role fallback (above) didn't help because the failure isn't shape-specific -- both `[system, user]` and `[merged user]` raise the same error. **Resolved properly via the vendored `jinja2` chat-template renderer** described in the `Added` section above: `LLM._apply_template` now tries `_apply_jinja_template` first, which evaluates the embedded Jinja template directly via the vendored `jinja2 3.1.6`, and falls back to the legacy substring-heuristic path on `TemplateError`. Gemma 4 chat now works end-to-end -- specifically, the prior workaround (a third-tier raw-completion fallback in `RAGPipeline._chat_with_fallback` with a one-time `RuntimeWarning`) is now mostly dead code: it remains as a final safety net but should not fire in practice for any model whose template `jinja2` can evaluate. The 4 tests in `tests/test_rag_pipeline.py::TestRAGPipelineRawCompletionFallback` continue to pin the third-tier fallback's behaviour for the rare case where it does fire (corrupted GGUFs, malformed Jinja templates, etc.)
