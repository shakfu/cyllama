...
- **Experimental abi3 (stable-ABI) wheel build path** -- New `CYLLAMA_ABI3` CMake option (default `OFF`) wires every Cython extension through CPython's limited API so a single `cp312-abi3-<plat>` wheel is importable on Python 3.12+. Implementation is entirely additive: bumped `cmake_minimum_required` to 3.26 (needed by `USE_SABI`), conditionally requests `Development.SABIModule` in `find_package(Python ...)`, and threads a `_SABI_ARGS` helper (`USE_SABI 3.12` or empty) through all four `python_add_library` calls (`add_cython_extension`, `embedded`, `whisper_cpp`, `stable_diffusion`). No `.pyx` files were modified and no Cython-level directive is needed — Cython's limited-API codegen is driven purely by the `Py_LIMITED_API` macro that `USE_SABI` defines at C compile time. Opt in locally with `pip install . --config-settings=cmake.define.CYLLAMA_ABI3=ON --config-settings=wheel.py-api=cp312` (or `make dev-abi3` / `make wheel-abi3`); in CI via `SKBUILD_CMAKE_DEFINE=CYLLAMA_ABI3=ON` + `SKBUILD_WHEEL_PY_API=cp312`. Default per-version build path is unchanged. Floor is 3.12: 3.10 fails to compile because `Py_buffer` / `PyBUF_*` entered the stable ABI only in 3.11 (PEP 688) and Cython's memoryview boilerplate references them; 3.11 would compile but misses vectorcall, which entered the limited API in 3.12 and is used by Cython's fast-call path for call-heavy hot loops (generation, samplers). 3.10 and 3.11 users continue to use the per-version build until EOL (3.10: Oct 2026, 3.11: Oct 2027). See `docs/dev/abi3.md` and https://cython.readthedocs.io/en/latest/src/userguide/limited_api.html.

- **Two experimental workflows for abi3 wheel builds** -- `.github/workflows/build-abi3.yml` (CPU, equivalent to `build-cibw.yml`) and `.github/workflows/build-gpu-wheels-abi3.yml` (CUDA/ROCm/SYCL/Vulkan Linux + CUDA/Vulkan Windows + Vulkan macOS-Intel, equivalent to `build-gpu-wheels2.yml`). Both set `CIBW_BUILD=cp312-*` and inject `SKBUILD_CMAKE_DEFINE="CYLLAMA_ABI3=ON"` + `SKBUILD_WHEEL_PY_API=cp312` into every `CIBW_ENVIRONMENT_*` block (per-platform, not generic — cibuildwheel's env resolution replaces pyproject.toml's `environment-<plat>` tables when a generic `CIBW_ENVIRONMENT` is set, which drops `MACOSX_DEPLOYMENT_TARGET=11.0` on macOS and breaks the llama.cpp `std::filesystem` build). Artifact names, cache keys, and release/index artifacts are namespaced with an `abi3` prefix so the flows coexist with the per-version workflows without collision. Smoke-test jobs add a platform-aware abi3 tag guard (`.abi3.so` on Linux/macOS; plain `.pyd` with no `cp3XX-` tag on Windows; `cyllama/rag/vector.*` exempted since sqlite-vector is a plain C shared library loaded by SQLite, not a CPython extension), catching the silent case where `SKBUILD_WHEEL_PY_API` re-tags a regular wheel without the CMake option actually activating. The CPU workflow additionally verifies imports + one-token decode on a 3.12 x 3.13 x 3.14 matrix across Linux/macOS-arm/Windows — cross-version validation that cibuildwheel itself cannot perform on a single abi3 wheel.

- **`make dev-abi3` and `make wheel-abi3` targets** -- Editable install and wheel-build convenience targets that wrap the two `--config-setting` flags for the abi3 opt-in path. Mirror the existing `make dev` and `make wheel` targets; output of `make wheel-abi3` is a single `dist/cyllama-<ver>-cp312-abi3-<plat>.whl`.
...
- **Linux GPU wheels shipped without project dynamic libs** -- `.github/workflows/build-gpu-wheels2.yml` passed `--exclude libllama.so.0 --exclude libggml*.so.0 --exclude libmtmd.so.0 --exclude libggml-<backend>.so` to `auditwheel repair` for all four backends (CUDA, ROCm, SYCL, Vulkan). Those are project libs, not system/driver libs. Excluding them told auditwheel "don't bundle" — the resulting wheels had extensions with `NEEDED libllama.so.0` but no `cyllama_<backend>.libs/` contents, breaking `import cyllama` with `ImportError: libllama.so.0: cannot open shared object file` on any host without llama.cpp installed system-wide. Repair excludes are now limited to true system/driver libs (`libcuda.so.1`, `libcudart.so.12`, `libcublas*.so.12`, `libamdhip64.so.6`, `libhipblas.so.2`, `librocblas.so.4`, `libhsa-runtime64.so.1`, `libsycl.so.8`, `libOpenCL.so.1`, `libvulkan.so.1`, `libgomp.so.1`), letting auditwheel auto-bundle the project libs via `RUNPATH` lookup as originally designed.

- **Recursive ccache invocation in GPU wheel builds** -- `CMakeLists.txt` set `RULE_LAUNCH_COMPILE=ccache` unconditionally whenever ccache was found on `PATH`, while `build-gpu-wheels2.yml` already sets `CMAKE_{C,CXX,CUDA,HIP}_COMPILER_LAUNCHER=ccache` via `CIBW_ENVIRONMENT_LINUX`. The stacked wrappers produced `ccache ccache gcc …`, which ccache rejects with `Recursive invocation`, failing every CUDA/ROCm/SYCL/Vulkan job at the first compile step. CMake now skips its own wrapping when `CMAKE_C_COMPILER_LAUNCHER` or `CMAKE_CXX_COMPILER_LAUNCHER` is already defined.
