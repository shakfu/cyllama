...
- **Experimental abi3 (stable-ABI) wheel build path** -- New `CYLLAMA_ABI3` CMake option (default `OFF`) wires every Cython extension through CPython's limited API so a single `cp311-abi3-<plat>` wheel is importable on Python 3.11+. Implementation is entirely additive: bumped `cmake_minimum_required` to 3.26 (needed by `USE_SABI`), conditionally requests `Development.SABIModule` in `find_package(Python ...)`, and threads a `_SABI_ARGS` helper (`USE_SABI 3.11` or empty) through all four `python_add_library` calls (`add_cython_extension`, `embedded`, `whisper_cpp`, `stable_diffusion`). No `.pyx` files were modified and no Cython-level directive is needed — Cython's limited-API codegen is driven purely by the `Py_LIMITED_API` macro that `USE_SABI` defines at C compile time. Opt in locally with `pip install . --config-settings=cmake.define.CYLLAMA_ABI3=ON --config-settings=wheel.py-api=cp311` (or `make dev-abi3` / `make wheel-abi3`); in CI via `SKBUILD_CMAKE_DEFINE=CYLLAMA_ABI3=ON` + `SKBUILD_WHEEL_PY_API=cp311`. Default per-version build path is unchanged. Floor is 3.11, not 3.10, because `Py_buffer` and `PyBUF_*` entered the stable ABI in 3.11 (PEP 688); Cython's auto-generated memoryview boilerplate references them, so a 3.10 floor fails to compile `whisper_cpp` and `stable_diffusion`. 3.10 users continue to use the per-version build until 3.10 EOL (Oct 2026). See `docs/dev/abi3.md`.

- **Two experimental workflows for abi3 wheel builds** -- `.github/workflows/build-abi3.yml` (CPU, equivalent to `build-cibw.yml`) and `.github/workflows/build-gpu-wheels-abi3.yml` (CUDA/ROCm/SYCL/Vulkan Linux + CUDA/Vulkan Windows + Vulkan macOS-Intel, equivalent to `build-gpu-wheels2.yml`). Both set `CIBW_BUILD=cp311-*` and inject `SKBUILD_CMAKE_DEFINE="CYLLAMA_ABI3=ON"` + `SKBUILD_WHEEL_PY_API=cp311` into every `CIBW_ENVIRONMENT_*` block (per-platform, not generic — cibuildwheel's env resolution replaces pyproject.toml's `environment-<plat>` tables when a generic `CIBW_ENVIRONMENT` is set, which drops `MACOSX_DEPLOYMENT_TARGET=11.0` on macOS and breaks the llama.cpp `std::filesystem` build). Artifact names, cache keys, and release/index artifacts are namespaced with an `abi3` prefix so the flows coexist with the per-version workflows without collision. Smoke-test jobs add an `.abi3.` suffix guard that asserts every installed `.so`/`.pyd` carries the abi3 soabi tag, catching the silent case where `SKBUILD_WHEEL_PY_API` re-tags a regular cp311 wheel without the CMake option actually activating. The CPU workflow additionally verifies imports + one-token decode on a 3.11 x 3.12 x 3.13 x 3.14 matrix across Linux/macOS-arm/Windows — cross-version validation that cibuildwheel itself cannot perform on a single abi3 wheel.

- **`make dev-abi3` and `make wheel-abi3` targets** -- Editable install and wheel-build convenience targets that wrap the two `--config-setting` flags for the abi3 opt-in path. Mirror the existing `make dev` and `make wheel` targets; output of `make wheel-abi3` is a single `dist/cyllama-<ver>-cp311-abi3-<plat>.whl`.
