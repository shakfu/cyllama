...
- **Stale `hashFiles()` paths in workflow cache keys** -- `build-cibw.yml:50` referenced `.github/workflows/build-cibw.yaml` (wrong extension); `build-cibw-abi3.yml:45` referenced `build-abi3.yml` (wrong name); three Windows/macOS-Intel jobs in `build-gpu-wheels.yml` (lines 531, 617, 700) referenced a non-existent `build-gpu-wheels2.yml`. `hashFiles()` returns an empty string for missing paths, so the affected cache keys collapsed to a stable prefix with no workflow-file component -- edits to the workflow (or `manage.py`, in the cibw cases) never invalidated the `thirdparty/` cache, silently serving stale third-party builds. All 17 `hashFiles()` refs now resolve. Also cleaned up 5 stale `build-gpu-wheels2.yml` mentions in prose comments (`build-new-wheels.yml:8,42`, `build-gpu-wheels-abi3.yml:3,6,1002`) left over from a rename.

### Changed

- **`scripts/manage.py` backend-option builders refactored** -- The three ~100-line `get_backend_cmake_options()` methods on `LlamaCppBuilder`, `WhisperCppBuilder`, and `StableDiffusionCppBuilder` duplicated the same GGML_* env-flag → CMake-option plumbing. Introduced an intermediate `GgmlBuilder(Builder)` class holding shared helpers (`_set_backend`, `_apply_cuda_extras`, `_apply_hip_archs`, `_apply_openmp`, `_forward_env_flags`, `CUDA_TUNING_ENV_FLAGS`) and the `get_backend_cmake_options` NotImplementedError stub. The three ggml-backed concrete builders now extend `GgmlBuilder` and each owns a self-contained, readable method that calls into the shared helpers; `SqliteVectorBuilder` continues to subclass `Builder` directly. Net -98 lines, no change to emitted CMake options under any GGML_* env combination. Also annotated `lib_exts` in `LlamaCppBuilder.download_release` as `tuple[str, ...]` to unblock `make qa` (mypy).
...
- **ccache on CPU cibw workflows** -- `hendrikmuhs/ccache-action@v1.2` now wired into `build-cibw.yml` and `build-cibw-abi3.yml` for Linux and macOS jobs, matching the pattern already in `build-gpu-wheels.yml` / `build-gpu-wheels-abi3.yml`. `CMAKE_C_COMPILER_LAUNCHER=ccache` and `CMAKE_CXX_COMPILER_LAUNCHER=ccache` are passed through `CIBW_ENVIRONMENT_{LINUX,MACOS}` alongside `CCACHE_DEPEND=1`; `CCACHE_DIR` points at `/project/.ccache` on Linux (the manylinux container's bind-mount of `${{ github.workspace }}/.ccache`) and at `${{ github.workspace }}/.ccache` on macOS. Cache keys are `ccache-cibw-<os>` and `ccache-abi3-<os>` with `evict-old-files: 1d`, scoped per-matrix-OS so Linux/macOS/Windows don't collide. ccache is installed inside the manylinux container via `pyproject.toml`'s `before-all` (`yum install -y git ccache`); on macOS runners `hendrikmuhs/ccache-action` installs it on the host when missing. Workflow-level `CIBW_ENVIRONMENT_<PLAT>` replaces (not merges with) `[tool.cibuildwheel.<plat>].environment`, so `build-cibw.yml`'s new override re-declares the pyproject defaults (`GGML_METAL=0`, `GGML_NATIVE=0`, `MACOSX_DEPLOYMENT_TARGET=11.0`) alongside the ccache vars. Windows MSVC is deliberately excluded -- ccache's PDB / cmake-integration caveats need a separate investigation. Biggest payoff on the cibuildwheel CPython-version axis: the same C/C++ source is compiled for cp310/311/312/313/314, so the second-through-fifth Python builds within a single run hit cache immediately.

- **Concurrency groups on CPU cibw workflows** -- `build-cibw.yml` and `build-cibw-abi3.yml` gain `concurrency: { group: ${{ github.workflow }}-${{ github.ref }}, cancel-in-progress: true }`, matching the pattern already in `build-gpu-wheels.yml`, `build-gpu-wheels-abi3.yml`, `build-new-wheels.yml`, and `check-vendor.yml`. A second push on the same ref now cancels the in-flight run instead of letting both the old and new matrices burn runner minutes to completion.
