...
- **llama.cpp bumped to `b8833`** -- Upstream renamed the `common` CMake target (and its static archive `libcommon.a`) to `llama-common` / `libllama-common.a`. Updated `scripts/manage.py` (`LlamaCppBuilder.libs_static`, static and shared `cmake_build_targets` lists, and the `copy_lib` call) and `Makefile` (`LLAMACPP_LIBS` on macOS) accordingly. Also noted header-level upstream changes that do not affect cyllama's bindings: `common.h` no longer exposes the `LLAMA_BUILD_NUMBER` / `LLAMA_COMMIT` / `LLAMA_COMPILER` / `LLAMA_BUILD_TARGET` externs, the `print_build_info()` macro, or the `build_info` string constant (replaced by a new `build-info.h` with `llama_build_number()` / `llama_commit()` / `llama_print_build_info()` accessor functions); `log.h` replaces the `extern common_log_verbosity_thold` global with a `common_log_get_verbosity_thold()` accessor; `ggml-backend.h` reshapes the tensor-parallel allreduce API from `ggml_backend_allreduce_tensor_t` into the `ggml_backend_comm_init_t` / `comm_free_t` / `comm_allreduce_tensor_t` trio with an explicit `comm_ctx` handle; `mtmd-helper.h` tags `mtmd_decoder_pos` with `struct` in `mtmd_helper_image_get_decoder_pos`; and `mtmd.h` drops the deprecated `MTMD_DEFAULT_IMAGE_MARKER` macro in favor of `mtmd_default_marker()`. Full `make test` passes on the bumped revision
