...
- **`docs/dev/chat_templates.md`** - Design memo capturing the analysis for upgrading cyllama's chat-template handling from llama.cpp's basic C API (`llama_chat_apply_template`, which only supports a hardcoded subset detected via substring heuristics) to the full Jinja-aware path (`common_chat_templates_apply` from `common/chat.cpp`, which uses `minja` to evaluate any embedded Jinja template).
...
- **`cyllama rag` crash on models whose embedded Jinja chat template can't be applied at all** - Some GGUFs (Gemma 4 in particular, plus any model whose embedded Jinja template doesn't match one of the substring heuristics in llama.cpp's `llm_chat_detect_template`) cause `llama_chat_apply_template` to return -1 regardless of message shape. This is because cyllama's Cython binding calls llama.cpp's basic C API `llama_chat_apply_template`, which only handles a hardcoded set of templates detected via substring heuristics -- it does NOT actually evaluate Jinja. There's a separate `common_chat_apply_template` in `common/chat.cpp` with full Jinja support via `minja`, but cyllama doesn't currently link against that. The earlier system-role fallback (above) didn't help because the failure isn't shape-specific -- both `[system, user]` and `[merged user]` raise the same error. Fixed by adding a third-tier fallback: when both chat shapes fail with a template error, the pipeline permanently degrades to the raw-completion path for the rest of its lifetime, with `gen_config` rebuilt to include the `Question:/Answer:` stop sequences that the completion path needs. A `RuntimeWarning` fires once per pipeline instance so the silent quality degradation isn't invisible to the user. The decision is cached on `self._chat_template_unusable` so subsequent queries skip the chat attempt entirely. Users hitting this can suppress the warning by passing `--no-chat-template` (or `RAGConfig(use_chat_template=False)`). Long-term fix is to update the Cython binding to call `common_chat_apply_template`, which would give us full Jinja support and let any model use its native chat format. 4 new tests in `tests/test_rag_pipeline.py::TestRAGPipelineRawCompletionFallback` cover: the fallback firing on double template failure, the cached decision skipping chat on subsequent queries, the rebuilt completion stop sequences, and unrelated-error propagation
